...
class PacienteNoEncontradoException(Exception):
    """Se ejecuta cuando un paciente no está registrado."""
    pass

class MedicoNoDisponibleException(Exception):
    """Se ejecuta cuando un médico no está registrado o no disponible."""
    pass

class TurnoOcupadoException(Exception):
    """Se ejecuta cuando ya existe un turno para ese médico en la misma fecha y hora."""
    pass

class RecetaInvalidaException(Exception):
    """Se ejecuta cuando se intenta emitir una receta sin medicamentos."""
    pass
...
class Clinica:
    ...
    def validar_existencia_paciente(self, dni: str):
        if dni not in self.__pacientes:
            raise PacienteNoEncontradoException(f"el Paciente con {dni}: no se ah encontrado.")

    def validar_existencia_medico(self, matricula: str):
        if matricula not in self.__medicos:
            raise MedicoNoDisponibleException(f"el Médico con {matricula}: no se ah encontrado.")
    ...
    def validar_especialidad_en_dia(self, medico: Medico, especialidad: str, dia_semana: str):
        ...
        if disponible is None or disponible.lower() != especialidad.lower():
            raise MedicoNoDisponibleException(
                f"{medico.obtener_matricula()} no da consulta {especialidad} los {dia_semana}"
            )
    ...
    def agregar_paciente(self, paciente: Paciente):
        if paciente is None:
            raise ValueError("El paciente no puede ser Nulo.")
        dni = paciente.obtener_dni()
        if dni in self.__pacientes:
            raise ValueError(f"Ya hay un paciente con ese {dni}.")
        self.__pacientes[dni] = paciente
        self.__historias[dni] = HistoriaClinica(paciente)

    def agregar_medico(self, medico: Medico):
        if medico is None:
            raise ValueError("El médico no puede ser Nulo.")
        matricula = medico.obtener_matricula()
        if matricula in self.__medicos:
            raise ValueError(f"Ya hay un medico con esa {matricula}.")
    ...
    def emitir_receta(self, dni: str, matricula: str, medicamentos: List[str]):
        ...
        if not medicamentos or any(not str(m).strip() for m in medicamentos):
            raise RecetaInvalidaException("Ponga por lo menos  un medicamento.")
